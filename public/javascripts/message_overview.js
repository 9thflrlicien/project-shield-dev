...
$(document).ready(function() {
  ...
  function pushMsg() {
    ...
    database.ref('users' + userId).on('value', snap => {
      ...
      if (profileData == null) console.log('profileData == null');
        ...
        chanId1 = profileData.chanId_1;
        chanId2 = profileData.chanId_2;
      }
    })
  ...
  function btnText() {
    $('#inputText').append('<div style="margin:2%">' + '<span style="float:right" onclick="this.parentElement.remove()">X</span>' + '<tr>' + '<th style="padding:1.5%; background-color: #ddd">Enter Text:</th>' + '</tr>' + '<tr>' + '<td style="background-color: #ddd">' + '<form style="padding:1%">' + '<input id="textinput" style="width:100%;height:100px" />' + '</form>' + '</td>' + '</tr>' + '</div>');
  }
    ...
    database.ref('message-overview/' + userId).on('value', snap => {
      ...
      for (var i = 0; i < myIds.length; i++) {
        ...
        if (dataArray[i].taskStatus == 'draft') {
          $('#data-draft').append('<tr class = "msgToSend">' + '<td id="' + myIds[i] + '" hidden>' + myIds[i] + '</td>' + '<td class="msgDetail" style="margin:0;width:5%">' + (i + 1) + '</td>' + '<td class="msgDetail" style="margin:0;width:20%">' + dataArray[i].taskContent + '</td>' + '<td class="msgDetail" style="margin:0;width:10%">' + 'text' + '</td>' + '<td class="msgDetail" style="margin:0;width:15%">-</td>' + '<td class="msgDetail" style="margin:0;width:10%">' + dataArray[i].taskStatus + '</td>' + '<td class="msgDetail" style="color:red; margin:0; width:10%">' + dataArray[i].taskTime + '</td>' + '<td style="margin:0;width:10%">' + '<a href="#" id="editBtn" data-toggle="modal" data-target="#editModal"><b>編輯  </b></a>' + '<a href="#" id="viewBtn" data-toggle="modal" data-target="#viewModal"><b>檢視  </b></a>' + '<a href="#" id="deleBtn"><b>刪除</b></a>' + '</td>' + '</tr>');
        } else {
          $('#data-appointment').append('<tr class = "msgToSend" style="margin:0;">' + '<td id="' + myIds[i] + '" hidden>' + myIds[i] + '</td>' + '<td class="msgDetail" style="margin:0;width:5%">' + (i + 1) + '</td>' + '<td class="msgDetail" style="margin:0;width:20%">' + dataArray[i].taskContent + '</td>' + '<td class="msgDetail" style="margin:0;width:10%">' + 'text' + '</td>' + '<td class="msgDetail" style="margin:0;width:15%">-</td>' + '<td class="msgDetail" style="margin:0;width:10%">' + dataArray[i].taskStatus + '</td>' + '<td class="msgDetail" style="color:red; margin:0; width:10%">' + dataArray[i].taskTime + '</td>' + '<td style="margin:0;width:10%">' + '<a href="#" id="editBtn" data-toggle="modal" data-target="#editModal"><b>編輯  </b></a>' + '<a href="#" id="viewBtn" data-toggle="modal" data-target="#viewModal"><b>檢視  </b></a>' + '<a href="#" id="deleBtn"><b>刪除</b></a>' + '</td>' + '</tr>');
        }
      }
    });
  }
  ...
  function ISODateString(d) {
    ...
    return d.getFullYear() + '-' + pad(d.getMonth() + 1) + '-' + pad(d.getDate()) + 'T' + '00:00'
  }
