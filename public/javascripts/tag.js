$(document).ready(function() {
  ...
  socket.on("push tags", data=> {
    console.log("data:");
    console.log(data);
    for( let i in data ) {
      append_new_tag();
      let type = data[i].type;
      tagTableBody.find(".tag-name:last").text(data[i].name);
      tagTableBody.find(".tag-option:last").val(type);
      tagTableBody.find('.tag-set-td:last').find('#set'+toTypeValue(type)).val(data[i].set).show().siblings().hide();
    }
  });
